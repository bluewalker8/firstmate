...
3. **Write within firstmate's existing write boundaries.**
   ...
   - Knowledge generalizable to every firstmate user: this repo's own `AGENTS.md` (or other shared, tracked material), shipped through the normal isolated branch -> self-review and targeted checks -> PR -> CI -> captain-merge pipeline for this repo (section 1), never hand-committed straight to `main`.
