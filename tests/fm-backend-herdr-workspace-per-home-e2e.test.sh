...
# bin/fm-brief-lint.sh (run by fm-spawn.sh before launch) hard-fails a brief
# that never states the exact absolute state/<id>.status path; these trivial
# fixtures state it so the real spawn machinery is exercised, not the lint.
cat > "$PRIMARY_HOME/data/cm1/brief.md" <<EOF
Trivial e2e primary crewmate brief: nothing to do.
Status protocol: report status by appending to $PRIMARY_HOME/state/cm1.status
Definition of done: nothing to do.
EOF
...
Status protocol: report status by appending to $SM_HOME/state/cm2.status
Definition of done: nothing to do.
EOF
