...
# for a leading `$<skill>` invocation. Submitting before the
...
first_settle 1.2 'codex $skill -> long settle' codex '$sample-skill'
...
first_settle 1.2 'codex $skill exact task id -> long settle' codex '$sample-skill' exact

# Same `$` message to claude keeps the fast path: `$` is ordinary text there.
first_settle 0.3 'claude $-message -> fast path' claude '$sample-skill'
...
first_settle 0.3 'explicit target $message -> fast path (unknown harness)' --explicit '$sample-skill'

# The `/` slash case stays universal and unchanged: long settle regardless of
# harness (here a non-codex claude target).
first_settle 1.2 'claude /command -> long settle (slash unchanged)' claude '/compact'
