...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and the fm-decisions.sh board
FM_DECISIONS_MARKER_RE='awaiting captain|captain decision|captain open|hold-kind:[[:space:]]*captain'   # fm-decisions.sh: case-insensitive backlog markers that flag a line as waiting on the captain
