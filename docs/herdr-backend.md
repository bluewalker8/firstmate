...
That verification did not cover a crew blocked on its own long-running foreground tool call, which can keep the harness synchronously busy for minutes until an outcome.
...
At the same time, the read-only compatibility lookup for an existing legacy task could miss this crew's branch when its status response reported another branch.
For safe reconciliation of those historical records, `bin/fm-crew-state.sh` retains a bounded coarse cross-branch lookup.
...
The compatibility fallback uses the historical run list, and the watcher checks provably-working evidence before a stale status-log verb can make a stale pane terminal.
...
Plain (non-argument) commands like `/new` did submit on the first Enter in the same live test, so the false-positive was specific to commands whose popup selection fills an argument placeholder rather than submitting outright.
