...
It also lints each ship or scout brief with `bin/fm-brief-lint.sh` before any worktree or backend setup, refusing the launch when the crewmate's status-append target is missing or written as a relative `state/<id>.status` path that would let the crewmate create `state/` inside its own project worktree and commit it in.
