...
| `fm-board.sh`            | Read-only captain-facing visual fleet board: one progress-bar line per in-flight task, grouped by project |
| `fm-board-bind.sh`       | Idempotently bind a tmux key (prefix+F, fallback prefix+B) to a hidden fleet-board popup |
