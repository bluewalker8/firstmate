...
2. **Never merge a PR without the captain's explicit word.**
   The one standing, captain-authorized relaxation is a project's `yolo` flag (section 7): with `yolo` on, firstmate makes routine approval decisions itself, but anything destructive, irreversible, security-sensitive, or client-money-touching still escalates to the captain.
...
This repo follows section 7's normal isolated-lane direct-PR contract, and the captain's merge rule applies here exactly as it does to projects.
...
**Delivery mode (record at add).** `<mode>` is how a finished change reaches `main`, picked per project when you add it and recorded in the registry line (`fm-project-mode.sh` parses it; `fm-spawn` records it into each task's meta):
...
Existing unbracketed registry entries continue to resolve as `no-mistakes` for parser compatibility and must not be silently reinterpreted; new entries always record their mode explicitly.

**Clone existing:** `git clone <url> projects/<name>`, then add its registry line with the chosen mode.

**Create new:** a `direct-PR` project needs a GitHub repo first because it pushes to an `origin` remote; a `local-only` project needs no remote at all - a purely local git repo is fine.
...
Have the crewmate rebase onto the authoritative base before its final self-review when the base advanced or mild overlap appeared.
...
The Validate and open PR / PR ready / Ship teardown stages below are written for the default `direct-PR` path; the other modes diverge:
...
When the task meta records `pr=`, the helper also compares that base against the authoritative PR head (`pr_head=` when reachable, otherwise a fresh `refs/pull/<n>/head` fetch) so updates pushed to the PR are included even if the local worktree branch is stale.
If the PR head cannot be resolved, it warns loudly and falls back to the local branch.
For an already-registered legacy `no-mistakes` project, commits under `.no-mistakes/evidence/` in a crew branch are the legacy pipeline's own PR-viewable validation evidence, committed by design so it rides along with the change.
...
The default `direct-PR` lane owns delivery from implementation through the opened PR.
Before reporting done, the crewmate must fetch the authoritative base, rebase when needed, and self-review the complete diff against that base line by line.
It fixes every valid finding in its lane, then runs the narrowest relevant tests plus typecheck and lint where the project provides them.
UI work requires browser verification of the changed behavior with a real screenshot, and every failed check is reported faithfully rather than softened or skipped.
...
Wait for every required PR check to pass before merging.
Merge ready PRs sequentially, never concurrently.
If the captain says "merge it", run `bin/fm-pr-merge.sh <id> <full GitHub PR URL>` yourself after checks pass; that instruction is the explicit approval.
...
After each confirmed merge, refresh the project's default branch and own integrated verification of the changed behavior before merging the next PR.
Use an isolated scout lane for integrated checks that may write project state, and browser-verify merged UI behavior with a real screenshot.
If integrated verification fails, report the failure with evidence and stop the merge sequence.

### Ship teardown (only after merge and integrated verification are confirmed)
...
- Afk never changes approval authority; PR merges, ask-user findings, destructive actions, irreversible actions, security-sensitive choices, and client-money-touching choices still require the same approval they required before.
...
- Anything destructive, irreversible, security-sensitive, or client-money-touching.
...
No brief may delegate decisions about PR merges or anything destructive, irreversible, security-sensitive, or client-money-touching unless the captain has already supplied the required authority.
...
Firstmate ships its own improvements to `AGENTS.md`, `bin/`, `.agents/skills/`, and public `skills/` through section 7's isolated-lane direct-PR contract, then waits for each running firstmate to pull the merged default branch.
...
It is not consent for destructive, irreversible, security-sensitive, or client-money-touching actions; those still require trusted-channel confirmation first.
